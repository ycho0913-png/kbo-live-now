      ...
      {games.map((game) => (
        ...
        >
          ...
          <div className="mt-4 grid gap-1 text-xs font-bold text-ink/55">
            <span>
              선발 {game.awayStartingPitcher || "발표 전"} vs{" "}
...
function ScoreboardMatchupSummary({ game }: { game: ScoreboardGame }) {
  ...
  return (
    <div className="mt-4 space-y-3 rounded-lg border border-line bg-paper p-3">
            ...
            {analysis.teamPower.slice(0, 2).map((team) => (
              <div key={team.team} className="rounded-md bg-white p-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs font-black text-ink">{team.record || "-"}</span>
                </div>
                <div className="mt-2 grid grid-cols-3 gap-1 text-center text-[11px] font-bold text-ink/55">
      ...
      {analysis.startingPitchers.length ? (
        <div>
          <p className="text-[11px] font-black text-[#ff6b00]">선발 투수 매치업</p>
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            {analysis.startingPitchers.slice(0, 2).map((pitcher) => (
              <div key={`${pitcher.team}-${pitcher.name}`} className="rounded-md bg-white p-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate text-sm font-black text-ink">{pitcher.name}</span>
                  <span className="text-[11px] font-bold text-ink/50">{pitcher.team}</span>
                </div>
                <div className="mt-2 grid grid-cols-3 gap-1 text-center text-[11px] font-bold text-ink/55">
                  <span>ERA {pitcher.era || "-"}</span>
                  <span>WHIP {pitcher.whip || "-"}</span>
                  <span>QS {pitcher.qualityStarts || "-"}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            {topPlayers.map((player, index) => (
              <div key={`${player.team}-${player.name}-${index}`} className="rounded-md bg-white p-2">
                ...
                <p className="mt-1 truncate text-[11px] font-bold text-ink/55">
