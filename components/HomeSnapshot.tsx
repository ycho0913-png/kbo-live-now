    ...
    <section className="px-3 py-5 sm:px-4">
      <div className="mx-auto grid max-w-6xl gap-3 lg:grid-cols-[1.1fr_0.9fr]">
        <Link
          ...
          className="rounded-lg border border-[#0b4ea2]/20 bg-white p-4 shadow-soft hover:border-[#0b4ea2]"
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-normal text-[#0b4ea2]">Samsung Focus</p>
              <h2 className="mt-2 text-xl font-black text-ink sm:text-3xl">삼성 라이온즈 바로보기</h2>
            </div>
            <span className="rounded-md bg-[#0b4ea2] px-3 py-2 text-lg font-black text-white">사자</span>
          </div>
          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
            <div>
              <p className="text-sm font-bold text-ink/55">{samsungGame?.date || "오늘 기준"} · {samsungGame?.time || "시간 확인중"}</p>
              <p className="mt-1 text-lg font-black text-ink">{gameLabel(samsungGame)}</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link href="/standings" className="rounded-lg border border-line bg-white p-4 shadow-soft hover:border-[#ff6b00]">
            <p className="text-xs font-black uppercase tracking-normal text-[#ff6b00]">Top 3</p>
            <div className="mt-3 grid gap-2">
              {topTeams.length ? (
                topTeams.map((row) => (
                  <div key={`${row.rank}-${row.team}`} className="flex items-center justify-between gap-2 text-sm font-black text-ink">
          ...
          <Link href="/news" className="rounded-lg border border-line bg-white p-4 shadow-soft hover:border-[#ff6b00]">
            <p className="text-xs font-black uppercase tracking-normal text-[#ff6b00]">News</p>
            <strong className="mt-2 block text-3xl font-black text-ink">{Math.min(news.length, 10)}</strong>
