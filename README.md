...
- Android Chrome에서는 사이트 접속 후 화면 하단의 `앱 버튼 만들기`를 누르면 홈 화면에 KBO NOW 아이콘을 추가할 수 있습니다.
- iPhone Safari에서는 공유 버튼을 누른 뒤 `홈 화면에 추가`를 선택하면 앱처럼 실행됩니다.
- 이 기능은 PWA manifest와 service worker를 사용하며, 실시간 API 응답은 앱 내부에 저장하지 않습니다.
